...
export class ReportsPage implements OnInit {
  ...
  async generateAllReport() {
    ...
    try {
      ...
      await this.savePDF(doc, `Hadaya_Report_${this.selectedMonth}.pdf`);
    } catch (e) {
      console.error(e);
  ...
  private drawHeader(doc: jsPDF, pageW: number, title: string, subtitle: string) {
    ...
    doc.setFontSize(28);
    doc.setTextColor(255, 255, 255);
    doc.text('Hadaya', pageW / 2, 30, { align: 'center' });
  ...
  private drawFooter(doc: jsPDF, pageW: number, pageH: number) {
    ...
    doc.text('GENERATED BY HADAYA', pageW / 2, pageH - 8, { align: 'center' });
    doc.setCharSpace(0);
